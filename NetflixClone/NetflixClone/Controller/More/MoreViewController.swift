...
class MoreViewController: UIViewController {
    ...
    private let tempprofileButton = UIButton(type: .system)
...
extension MoreViewController {
    ...
    private func testUI() {
        ...
        view.addSubview(tempprofileButton)
        tempprofileButton.setTitle("프로필", for: .normal)
        tempprofileButton.tintColor = .white
        tempprofileButton.addTarget(self, action: #selector(didTapProfileButton), for: .touchUpInside)
    ...
    private func testConstraint() {
        ...
        tempprofileButton.frame.size = CGSize(width: 80, height: 40)
        tempprofileButton.center.x = view.center.x
        tempprofileButton.center.y = tempVideoButton.center.y - 100
    }
